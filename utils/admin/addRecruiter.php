...
function updateRecruiterVerification($recId){
    global $conn;
    $recId = mysqli_real_escape_string($conn, $recId);
    $query = "UPDATE `recruiter` SET `isVerified`='1' WHERE `recId`='${recId}'";
    $result = mysqli_query($conn, $query);
...
if(isset($_GET['recId'])){
    ...
    if(!updateRecruiterVerification($_GET['recId'])){
        header('Location: /jobseeker/pages/admin/manage-recruiters.php?msg=Failed%20to%20update%20record.');
    }

    header('Location: /jobseeker/pages/admin/manage-recruiters.php?msg=Recruiter%20verified%20successfully.');
